...
class FileNode:
    def __init__(self,name,isFile,path):
        self.name = name
        print "init type of path", type(path)
        path = path.encode('ascii','ignore')
        print "init type of path 2", type(path)   
        self.path = path
    ...
    def put(self,key,value):
        key = self.path+"&&"+key
        print "type of key in put", type(key)
        print "NOTE key:" 
        print key
        print "NOTE value:" 
        print value
        ...
        #from bson.binary import Binary
        #import pickle
        #Binary_key = pickle.dumps(key)
        Binary_value = pickle.dumps(value)
        db.user2.insert({'key-data': Binary(key), 'value-data':Binary(Binary_value)})
        cursor=db.user2.find()
        for i in cursor:
            print "VALUE IN DATABASE:    :", i
        #rpc.put(Binary(key), Binary(pickle.dumps(value)), 6000)
        #cursor = db.user.find({"path":self.path, "key":key, "value":value, "name":self.name, "isFile":self.isFile})
        #cursor = db.user.find({"Binary(key)":Binary(key), "Binary(pickle.dumps(value))":Binary(pickle.dumps(value))})
        #print "NOTE put value:",cursor
        #if (cursor == None):
            #db.user.insert({path:self.path, key:key, value:value, name:self.name, isFile:self.isFile})
             
            #db.user.insert({"Binary(key)":Binary(key), "Binary(pickle.dumps(value))":Binary(pickle.dumps(value))})
    ...
    def get(self,key):
        ...
        print "NOTE get key:" 
        print key            
        cursor = db.user2.find({'key-data': Binary(key)}).sort( [['_id', -1]] ).limit(1)
        if (cursor!= None):
            for sample in cursor:
                print "NOTE get getResult:", pickle.loads(Binary(sample['value-data']))
            #for res in cursor:
                return pickle.loads(Binary(sample['value-data']))
        
        #getResult=db.user.find({key:key})
        #entries = getResult[:]
        
        #print "NOTE get getResult:",entries
        #print "NOTE get getResult:",getResult
        #getResult = db.user.find({"Binary(key)":Binary(key)})
        #if "Binary(pickle.dumps(value))" in getResult:
        #if "value" in getResult["value"]:
        #if  (getResult["value"] != None):
            #print "NOTE get if value: %s" %(pickle.loads(getResult["Binary(pickle.dumps(value))"].data))
            #print "NOTE get if value: %s" %getResult
    ...
    def add_node(self,newnode):
        ...
        print "NOTE add_node list_nodes: %s , newnode : %s" %(list_nodes,newnode)
    ...
    def contains_node(self,name): # returns node object if it exists
        if (self.isFile==True):
            ...
            if name in self.get("list_nodes").keys():
                print "NOTE contains_node if name: %s" %(self.get("list_nodes").keys()) 
...
class FS:
    ...
    def get_node(self,parent,PATH,name):
        print "get_node: %s" %PATH[1]
        next_node = parent.contains_node(PATH[1])
        print "NOTE get_node: %s" %next_node
    ...
    def get_parent_node(self,path):
        ...
        print "NOTE get_parent_node: %s" %parent_path
        parent_node = self.get_node_wrapper(parent_path)
        return parent_node

    def add_node(self,node,path):
        parent_path = "/"+("/".join(path.split('/')[1:-1]))
        print "NOTE add_node: %s" %parent_path
        parent_node = self.get_node_wrapper(parent_path)
    ...
    def add_dir(self,path,mode):
        ...
        print "NOTE  add_dir temp_node: %s" %temp_node
    ...
    def add_file(self,path,mode):
        ...
        print "NOTE  add_file temp_node: %s" %temp_node
    ...
    def write_file(self,path,data=None, offset=0, fh=None):
        ...
        print "NOTE  write_file filenode: %s" %filenode
        ...
        if (data==None):
            ...
            print "NOTE  write_file if node_data: %s" %node_data
            node_meta['st_size'] = offset
        else:
            node_data = node_data[:offset]+data
            print "NOTE  write_file else node_data: %s" %node_data
    ...
    def read_file(self,path,offset=0,size=None):
        # get file node
        filenode = self.get_node_wrapper(path)
        print "NOTE  read_file filenode: %s" %filenode
    ...
    def rename_node(self,old,new):
        ...
        print "NOTE  rename_node future_parent_node: %s" %future_parent_node
        ...
        print "NOTE  rename_node list_nodes: %s" %list_nodes
    ...
    def utimens(self,path,times):
        filenode = self.get_node_wrapper(path)
        print "NOTE  utimens filenode: %s" %filenode
        now = time()
    ...
    def delete_node(self,path):
        ...
        print "NOTE  delete_node parent_filenode: %s" %parent_filenode
        # get node to be deleted
        filenode = self.get_node_wrapper(path)
        print "NOTE  delete_node filenode: %s" %filenode
        # remove node from parents list
        list_nodes = parent_filenode.get("list_nodes")
        print "NOTE  delete_node list_nodes: %s" %list_nodes
        del list_nodes[filenode.name]
        parent_filenode.put("list_nodes",list_nodes)
    ...
    def link_nodes(self,target,source):
        ...
        print "NOTE  link_nodes temp_node: %s" %temp_node
    ...
    def update_meta(self,path,mode=None,uid=None,gid=None):
        ...
        filenode = self.get_node_wrapper(path)
        print "NOTE  update_meta filenode: %s" %filenode
        # if chmod request
        meta = filenode.get("meta")
        if (uid==None):
...
class Memory(LoggingMixIn, Operations):
    ...
    def getattr(self, path, fh=None):
        ...
        print "NOTE file_node getattr: %s" %file_node
        if (file_node == None):
            raise FuseOSError(ENOENT)
        else:
    ...
    def readdir(self, path, fh):
        ...
        file_node =  self.FS.get_node_wrapper(path)
        print "NOTE file_node readdir: %s" %file_node
